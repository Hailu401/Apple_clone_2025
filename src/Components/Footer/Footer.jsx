...
function Footer() {
  useEffect(() => {
    function handleFooterLinks() {
      ...
      const lines = document.querySelectorAll(".footer-links-wrapper h3");

      lines.forEach(line => {
        line.removeEventListener('click', toggleList); // Remove previous listener to avoid duplicates

        if (isMobile) {
          line.addEventListener("click", toggleList);
        } else {
          const ul = line.nextElementSibling;
          if (ul && ul.style) {
            ul.style.display = 'block'; // Ensure lists are visible on larger screens
          }
          line.classList.remove("closeSign"); // Remove close sign if it was added
        }
      });
    }
    ...
    function toggleList() {
      ...
      this.classList.toggle("closeSign");
    }

    // Initial call to set up based on initial screen size
    handleFooterLinks();

    // Listen for window resize events to adjust responsiveness
    window.addEventListener('resize', handleFooterLinks);

    // Clean up the event listener when the component unmounts
    return () => {
      window.removeEventListener('resize', handleFooterLinks);
      const lines = document.querySelectorAll(".footer-links-wrapper h3");
      lines.forEach(line => {
        line.removeEventListener('click', toggleList);
      });
    };
